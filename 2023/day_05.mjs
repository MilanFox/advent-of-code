...
const isNumberInSeeds = (num) => {
  return inputData.seedPairs.some(([start, range]) => num >= start && num < start + range)
}
...
const findSeed = (location) => {
  ...
  for (let i = 0; i < inputData.maps.length; i++) {
    value = findPrevValue(value, i);
  }
