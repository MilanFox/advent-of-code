...
const map = inputFile.split(lineBreak).filter(Boolean)
...
const findGearsInLine = (lineNumber) => {
  ...
  [...line].forEach((char, index) => {
    ...
    /* before */ foundGear.adjacent.push(...allPartNumbers.filter(part => part.line === lineNumber && part.end === index - 1));
    /* after */ foundGear.adjacent.push(...allPartNumbers.filter(part => part.line === lineNumber && part.start === index + 1));
    /* above */ foundGear.adjacent.push(...allPartNumbers.filter(part => part.line === lineNumber - 1 && arrayRange(part.start, part.end).some(numberIndex => [index - 1, index, index + 1].includes(numberIndex))))
    /* below */ foundGear.adjacent.push(...allPartNumbers.filter(part => part.line === lineNumber + 1 && arrayRange(part.start, part.end).some(numberIndex => [index - 1, index, index + 1].includes(numberIndex))))
